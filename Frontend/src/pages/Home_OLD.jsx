  ...
  useEffect(() => {
    const fetchHomeData = async () => {
      try {
        ...
        const trendingResponse = await animeService.getTopAnime({ limit: 10, offset: 5 });
        console.log('Trending response:', trendingResponse);
        if (trendingResponse.success && trendingResponse.data) {
          setTrendingAnime(trendingResponse.data);
        }

        // Fetch latest anime (currently airing)
        const latestResponse = await animeService.searchAnime({
          status: 'airing',
          order_by: 'start_date',
          sort: 'desc',
          limit: 18,
        ...
        // Fetch top rated for grid
        const topResponse = await animeService.getTopAnime({ limit: 24 });
    ...
    const interval = setInterval(() => {
      setCurrentSpotlight((prev) => (prev + 1) % Math.max(spotlightAnime.length, 1));
    }, 5000);
